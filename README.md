...
Rust wurde Go vorgezogen. Beide liefern einzelne Linux-Binaries und gute HTTP-Server. Fuer VaultLinks kritischste Bereiche - Pfad- und Dateiverarbeitung, nebenlaeufige Zaehler, Multipart-Streaming und langlebige Sessions - bietet Rust zusaetzliche Speicher- und Typensicherheit ohne Garbage-Collector.
...
│   ├── proxy.rs            vertrauenswuerdige Proxy-Header
│   ├── runtime.rs          SQLite-Overrides fuer Policy-Settings
...
SQLite ist bewusst gewaehlt: eindeutige Aliase, parallele Sessions, atomare Downloadlimits und crash-feste Transaktionen sind Kernanforderungen. WAL ist aktiv. Tabellen: `admins`, `sessions`, `shares`, `public_unlock_sessions`, `public_preview_sessions`, `runtime_settings`, `audit`.

`shares.max_upload_size` ist optional; `NULL` nutzt das globale Runtime-Limit. Migrationen laufen transaktional ueber `PRAGMA user_version`; unbekannte neuere Schemas verweigern den Start. Die Datenbank liegt standardmaessig in `/var/lib/vaultlink/data.sqlite` und muss `vaultlink:vaultlink 0600` gehoeren.
...
| `/admin/preview/raw` | GET/HEAD | Raw-Bild/PDF-Preview fuer Admins |
...
| `/admin/shares/:id/delete` | POST | Share loeschen |
...
| `/v/:token` | GET | oeffentliche Datei-/Ordnerseite |
| `/v/:token/unlock` | POST | passwortgeschuetzte Freigabe entsperren |
| `/v/:token/preview` | GET | oeffentliche Preview, zaehlt als Downloadzugriff |
...
| `/v/:token/download.zip` | GET | limitierter ZIP-Download fuer Ordner |
| `/v/:token/upload` | POST | exklusiver Ordnerupload |
| `/s/:alias` | GET | validierter Kurzlink |

Es gibt absichtlich keine oeffentliche JSON-API. Interne absolute Pfade werden nie gerendert.
...
Oeffentliche Ordnerfreigaben mit Downloadrecht bieten Breadcrumbs, Suche, ZIP, Download und Preview. `download_upload` erlaubt Upload in den aktuell navigierten Unterordner. `upload_only` zeigt keine Dateinamen.
...
- Public Media-Raw-Preview benoetigt einen kurzlebigen, share- und pfadgebundenen Preview-Token. Die Preview-Seite zaehlt genau einmal als Downloadzugriff; PDF-Range-Requests umgehen Downloadlimits nicht mehrfach.
...
VaultLink lauscht lokal, z. B. auf `127.0.0.1:8080`; Caddy oder Nginx terminiert HTTPS. Forwarded-Header werden nur aus `trusted_proxies` akzeptiert. Fuer Nginx/Nginx Proxy Manager bei grossen Uploads:
...
Die aktuelle Staging-VM `192.168.1.240` laeuft hinter Nginx; Built-in-ACME darf dort nicht aktiviert werden, solange Nginx Port 443 terminiert.

### Standalone TLS mit PEM-Dateien

`certificate_source = "files"` liest `cert_file` und `key_file`. Mit `reload_on_cert_change = true` laedt `systemctl reload vaultlink` die PEM-Dateien per SIGHUP neu. Fehlerhafte neue PEMs lassen die alte TLS-Konfiguration aktiv.

Fuer Port 443 ohne Root:
...
`certificate_source = "letsencrypt"` nutzt `rustls-acme` mit `tls-alpn-01` auf Port 443. Es gibt keine OS-Abhaengigkeit auf certbot, acme.sh oder Nginx. Der ACME-Cache liegt unter `data_directory`, z. B. `/var/lib/vaultlink/acme`, und enthaelt Account-/Zertifikatsdaten.
...
ZeroSSL/acme.sh bleibt als externe Alternative dokumentiert und ist vor allem fuer Reverse-Proxy- oder PEM-Standalone-Betrieb sinnvoll. EAB-Credentials gehoeren in `/etc/vaultlink/zerossl.env` mit `0600`, niemals in Logs oder systemd-Argumente.
...
- `cargo check --locked`: gruen
- `cargo fmt --all -- --check`: gruen
- `cargo clippy --locked --all-targets --all-features -- -D warnings`: gruen
...
Firewall: bei Reverse Proxy nur 80/443 fuer Caddy/Nginx oeffnen und VaultLink auf Loopback lassen. Bei Standalone nur 443 oeffnen.
