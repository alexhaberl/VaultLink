...
- SQLite and complete upgrade/rollback backups contain reusable share tokens and TOTP seeds. VaultLink does not encrypt these values with a separate application master key; database files and backups are therefore production credentials and must remain restricted to audited root/service access, encrypted storage and protected backup handling.
