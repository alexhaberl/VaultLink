...
rustls-webpki to 0.103.15. That fix is included in the published 0.7.1 packages.
...
- Push and pull-request CI audits the shared workspace `Cargo.lock`, compiles every fuzz target, replays curated inputs and runs a 30-second campaign per target, and runs setup, API, package, signed-update, upgrade, and rollback Docker smokes without external runtime networking. Weekly/manual native fuzz campaigns reuse validated main-branch corpora and retain logs, minimized inputs, and crash evidence; separate source-coverage reports describe the exercised production paths.
- Fresh RustSec audits gate soak start and final release publication; daily audits check `main` and the frozen candidate reported by the soak host. Findings and audit failures block those transitions without discarding soak evidence. The [soak protocol](docs/SOAK-RUNNER.md#start-collection-and-release-binding) documents candidate freeze, independent security status, and audit retries.
- Native CI runs checksum-pinned Gitleaks 8.30.1 over the complete fetched Git history with redacted output, recursive decoding, and archive inspection. `.gitleaksignore` contains only two commit-bound findings for the public RFC 6238 Appendix B TOTP test vector.
