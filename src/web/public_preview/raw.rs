...
use crate::{
    ...
    services::public_transfer::{
        ...
        PublicTransferClient, PublicTransferError,
    },
    PublicTransferRouteState,
};

use crate::web::{
    shares::PreviewRawQuery, transfer_runtime::set_transfer_cookie, AppError, Result,
...
async fn validate_preview_token(
    ...
    let preview_token =
        preview_token.ok_or(AppError(StatusCode::FORBIDDEN, "Preview token missing"))?;
    ...
    let valid = database(state.db().clone(), move |database| {
        database.preview_session(&preview_token, share_id, &token_path)
    })
    .await?;
    if valid {
        Ok(())
