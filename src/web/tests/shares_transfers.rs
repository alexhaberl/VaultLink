...
async fn html_and_api_share_search_validate_original_unicode_and_keep_empty_searches() {
    ...
    state
        .db()
        .create_session("search-session", 1, "csrf", Utc::now() + Duration::hours(1))
        .unwrap();
            ...
            for path in ["/admin/shares", "/api/v2/shares"] {
                ...
                req.headers_mut().insert(
                    ...
                    HeaderValue::from_str(&format!(
                        "vaultlink_session=search-session; vaultlink_locale={locale}"
                    ))
                    .unwrap(),
                );
                ...
                if expected == 400 && path == "/admin/shares" {
                    assert!(body.contains(if locale == "de" {
