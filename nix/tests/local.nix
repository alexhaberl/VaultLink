...
in pkgs.testers.runNixOSTest {
  ...
  testScript = ''
    ...
    machine.succeed("grep -Fq 'data_directory = \"/var/lib/vaultlink/second\"' /etc/vaultlink/second.toml")
    machine.fail("runuser -u vaultlink -- ${vaultlink}/bin/vaultlink --config /etc/vaultlink/second.toml > /tmp/vaultlink-second.log 2>&1")
    machine.succeed("grep -q 'Error: Contended.*\\.vaultlink-instance.lock' /tmp/vaultlink-second.log || { cat /tmp/vaultlink-second.log; exit 1; }")
