...
in pkgs.testers.runNixOSTest {
  ...
  testScript = ''
    ...
    server.succeed("install -d -o vaultlink -g vaultlink -m 0700 /srv/vaultlink/.vaultlink-internal /srv/vaultlink/.vaultlink-internal/uploads /srv/vaultlink/.vaultlink-internal/tombstones")
    ...
    client.succeed("grep -q 'already held' /tmp/vaultlink-second.log")
    client.succeed("systemctl restart vaultlink.service")
    client.wait_until_succeeds("curl --fail --silent http://127.0.0.1:8080/api/v2/health/ready | grep -q '\"ok\":true'")
    client.succeed("systemctl stop vaultlink.service")
    client.succeed("runuser -u vaultlink -- mkdir -m 0700 /mnt/storage/data")
    ...
    client.succeed("grep -q 'SQLite.*separate filesystem\|SQLite/WAL requires local' /tmp/vaultlink-remote-sqlite.log")
