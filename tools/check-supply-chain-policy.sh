...
fresh_schema_security_test='db::tests::fresh_database_is_exactly_schema_eleven_without_plaintext_secret_columns'
if ! grep -F -q "fresh_schema_test='$fresh_schema_security_test'" Makefile \
    ...
    report "security-test must fail closed unless the exact fresh schema-11 secret-column test exists and runs"
fi
