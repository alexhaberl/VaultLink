...
if [ "$(python3 tools/package-targets.py ids --allow-unprovisioned 2>/dev/null | wc -l)" -ne 9 ] \
    || [ "$(python3 tools/package-targets.py assets 0.7.2 --allow-unprovisioned 2>/dev/null | wc -l)" -ne 9 ]; then
    report "the package manifest must render exactly nine target IDs and nine unique 0.7.2 assets"
fi
...
if ! grep -F -q 'libguestfs-tools' deploy/docker/Dockerfile.qemu-runner \
    ...
    || ! grep -F -q "inputs.target_id == 'fedora44-arm64' && 240 || (inputs.target_id == 'ubuntu2404-arm64' || inputs.target_id == 'ubuntu2604-arm64') && 150 || 90" .github/workflows/distro-vm-images-refresh.yml \
    || ! grep -F -q 'if [ "$target_id" = ubuntu2404-arm64 ] || [ "$target_id" = ubuntu2604-arm64 ]; then' "$vm_provisioner" \
...
if ! grep -F -q 'REAL_UPDATE_NEW_VERSION: 0.7.3' "$package_workflow" \
    || ! grep -F -x -q 'REAL_PACKAGE_NEW_VERSION ?= 0.7.3' Makefile \
    || ! printf '%s\n' "$package_build_job" \
...
fresh_schema_security_test='db::tests::fresh_database_is_exactly_schema_twelve_without_plaintext_secret_columns'
if ! grep -F -q "fresh_schema_test='$fresh_schema_security_test'" Makefile \
    ...
    report "security-test must fail closed unless the exact fresh schema-12 secret-column test exists and runs"
fi
