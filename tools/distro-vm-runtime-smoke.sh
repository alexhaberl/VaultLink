...
install -d -o vaultlink -g vaultlink -m 0750 /mnt/storage /var/lib/vaultlink
cp -a "$api_work/root/." /mnt/storage/
cp -a "$api_work/data/." /var/lib/vaultlink/
install -d -o vaultlink -g vaultlink -m 0750 /mnt/.vaultlink-internal
if [ -d "$api_work/root/.vaultlink-internal" ]; then
    cp -a "$api_work/root/.vaultlink-internal/." /mnt/.vaultlink-internal/
fi
rm -rf /mnt/storage/.vaultlink-internal
chown -R vaultlink:vaultlink \
    /mnt/storage /mnt/.vaultlink-internal /var/lib/vaultlink
chmod 0750 /mnt/storage /mnt/.vaultlink-internal /var/lib/vaultlink
...
sed \
    ...
    -e 's/^expected_filesystem_type = ""$/expected_filesystem_type = "ext4"/' \
    -e 's|^expected_mount_source = ""$|expected_mount_source = "/dev/vdb"|' \
    -e "s|^internal_directory = \"$api_work/root/.vaultlink-internal\"$|internal_directory = \"/mnt/.vaultlink-internal\"|" \
    -e "s|$api_work/root|/mnt/storage|g" \
    -e "s|$api_work/data|/var/lib/vaultlink|g" \
    "$api_work/config.toml" >"$evidence/config.toml"
grep -F -x -q 'mode = "reverse_proxy"' "$evidence/config.toml"
grep -F -x -q 'enabled = true' "$evidence/config.toml"
grep -F -q 'trusted_proxies = [' "$evidence/config.toml"
grep -F -x -q 'require_mount = true' "$evidence/config.toml"
grep -F -x -q 'expected_filesystem_type = "ext4"' "$evidence/config.toml"
grep -F -x -q 'internal_directory = "/mnt/.vaultlink-internal"' "$evidence/config.toml"
[ "$(findmnt -n -o FSTYPE --target /mnt/storage)" = ext4 ]
[ "$(findmnt -n -o SOURCE --target /mnt/storage)" = /dev/vdb ]
...
for attempt in $(seq 1 120); do
    if curl --fail --silent --show-error \
        http://127.0.0.1:18081/api/v2/health/ready >"$evidence/readiness.json"; then
        break
    fi
...
install -d -o vaultlink -g vaultlink -m 0750 /mnt/storage/vaultlink-load/uploads
runuser -u vaultlink -- truncate -s 50G /mnt/storage/vaultlink-load/sparse-50GiB.bin
create_share() {
    path=$1
    permission=$2
