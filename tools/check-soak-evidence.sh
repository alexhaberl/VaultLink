...
[ "$(value expected_version)" = 0.7.1 ] || { echo "soak did not exercise VaultLink 0.7.1" >&2; exit 1; }
...
if ! grep -F -x -q "SOAK_COMMIT_SHA=$expected_commit" "$unit_env" \
    ...
    || ! grep -F -x -q 'SOAK_EXPECTED_VERSION=0.7.1' "$unit_env" \
...
[ "$(cat "$evidence/health.json")" = '{"ok":true,"version":"0.7.1"}' ] \
