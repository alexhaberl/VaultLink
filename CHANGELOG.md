...
**Withdrawn and unsupported on 2026-09-17.** The immutable packages omit the
merged rustls handshake-validation security update. Do not install or deploy
them; wait for a later supported release.
...
- Advanced the stable toolchain, canonical Docker smoke image, and native package-builder recipe to Rust 1.98.1; all nine builder-image locks are deliberately left `UNPROVISIONED` until the protected refresh emits reviewed replacements.
