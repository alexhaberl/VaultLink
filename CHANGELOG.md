...
- Added a fixed-boundary, root-only CIFS provisioner plus token-protected setup discovery that can select and prefill active hardened SMB mounts without granting the browser setup process mount privileges, with an explicit last-writer-wins opt-in for Replace uploads alongside external SMB clients.
