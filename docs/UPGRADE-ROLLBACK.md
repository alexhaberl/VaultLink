...
Inventory the old visible share root before creating anything. Resolve every existing `.vaultlink-internal`, case-insensitive/trailing-dot/space alias of that name, old `.vaultlink-*.part` upload fragment and delete-tombstone collision as user data; never let 0.4.1+ mistake an old user entry for its private namespace. User data remains directly in the share root. Verify counts, paths and content hashes against the snapshot before changing the SMB client mount or starting VaultLink.

Provision the reserved `.vaultlink-internal/{uploads,tombstones}` child only after the inventory is clean. Use a dedicated VaultLink SMB account. Co-writers receive the required Modify rights for user data in the share root but no administrative rights. Server ACLs must deny them read, write, delete, rename, parent `DELETE_CHILD`, ACL/owner changes (`WRITE_DAC`/`WRITE_OWNER`) and chmod/chown/setfacl-equivalent access to the internal tree. Then install the hardened mount unit/drop-in and prepare a **separate candidate configuration** with:
...
The serving process derives one canonical lifetime-lock domain from the visible root. A CIFS root may use the direct reserved child `<root_mount_path>/.vaultlink-internal`; its name is blocked from every user path and symlink traversal is disabled. Other required mounts use the private sibling `<root-parent>/.vaultlink-internal`; development uses `<root_mount_path>/.vaultlink-internal`. Rename or migrate a differently named legacy private directory while every VaultLink process is stopped, and update the candidate configuration before running the upgrade. Never create a second private directory to bypass lock contention.
