...
4. Sign and publish the native immutable release with its existing 21 assets.
   ...
   BuildKit, pushes by digest, assembles the `vX.Y.Z` multiarch index and
...
5. Confirm `linux/amd64` and `linux/arm64` on the **top-level** GHCR index,
   verify version and source revision labels on both platforms, and confirm
   an unauthenticated Linux host can pull the digest. The first GHCR package
   may need its visibility set to public by the repository owner. Keep ingress
   closed until the image is publicly pullable and both platform checks pass.
