...
If a file operation is already visible in the filesystem, later audit or storage durability uncertainty is not reported as a failed operation. Upload JSON uses the ordered `warnings` array to distinguish `storage_durability_uncertain` from `audit_durability_uncertain`; the deprecated singular `warning` field remains an aggregate compatibility signal. The queue checks the registered operation status after a lost, unreadable, or contradictory response and never resends automatically. Public upload directory audit entries have no guaranteed later backfill, so upload warnings ask the user to check the result manually. Rename/delete operations remain in the unchanged SecureFS journal and are completed once as actor `system` without a client IP.
...
- In reverse-proxy mode, VaultLink authenticates the connection through a protected Unix-socket peer UID or an mTLS client certificate before evaluating forwarding headers. `trusted_proxies` only restricts additional intermediate hops in an authenticated forwarding chain. A single validated client IP is reused for limits and audit records.
...
- Audit data is stored in a SQLite index with a periodic retention target of 100,000 rows. Cleanup runs hourly, so the index can exceed that target between runs. Its optional structured journald mirror and DB timing diagnostics use one separate telemetry worker with a queue of 64 events. A slow output sink cannot block database admission or retain transfer capacity; a full queue drops optional output and reports cumulative loss when the sink recovers. Controlled shutdown waits at most 250 ms for this output. Optional audit fields are limited to 1,024 UTF-8 bytes each and marked when truncated; the required SQLite records retain their existing bounds and transactional guarantees. SQLite retention removes routine events before security-priority events and warns if capacity pressure reaches security events; priority controls eviction order but is not indefinite or tamper-proof retention. journald has an independent host retention policy that operators must size for their forensic requirements. Passwords, TOTP secrets, session tokens, Share tokens, and client IPs are not written to journald.
...
Text previews use an extension allowlist and escaped `<pre>` output. Image previews use an allowlist, fixed content types, and `nosniff`. PDF previews are served as `application/pdf` inline without server rendering. Public raw previews require a short-lived Share/path-bound token. Counted transfers are committed before the first nonempty chunk is released, or at completion for an empty stream; an interrupted transfer can already count. Range requests share a fixed 15-minute resume grant that repeated requests cannot extend indefinitely.


Public transfer leases revalidate the current password policy and the independent unlock session in the same SQLite transaction that starts the lease, before reusing a resume grant. HEAD availability uses the same authorization predicate without creating a lease. Already-opened leases may finish after a password change.

Response capacity is shared by the request, response body and associated blocking file workers. A cancelled HTTP request does not release the global, peer or public stream slots while its open, read, seek, ZIP or preview work is still running. Workers use the already acquired capacity; they do not acquire another stream slot. Directory enumeration and metadata failures propagate through lists, search, snapshots and ZIP planning. Explicit private-name and confinement filters remain hidden. ZIP generation limits each source to its planned size and rejects observable growth or shrinkage before completing the archive; equal-size concurrent edits are not a filesystem snapshot.
