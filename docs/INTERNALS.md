...
If a file operation is already visible in the filesystem, later audit or storage durability uncertainty is not reported as a failed operation. Upload API and queue clients receive `202` with `audit_durability_uncertain`; browsers display a warning covering either cause. Clients must not retry that response automatically, even if its JSON body is incomplete. Public upload directory audit entries have no guaranteed later backfill, so upload warnings ask the user to check the result manually. Rename/delete operations remain in the unchanged SecureFS journal and are completed once as actor `system` without a client IP.
