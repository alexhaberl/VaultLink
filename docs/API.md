...
All three health routes require no application session. In reverse-proxy mode, a separate listener on `127.0.0.1:8082` serves only liveness and readiness; it does not expose application routes or accept forwarding headers. The production listener still requires an authenticated Unix peer or mTLS client before HTTP. Liveness does not touch SQLite or storage. Readiness returns `503` with `{"ok":false,"version":"..."}` until the production listener and dependencies are ready, while details are written only to structured logs. Operators and orchestrators can use `vaultlink health-check --live` and `--ready` locally.
